...
if (!Script.preview) {
  ...
  const binaryPkgbuild = [
    ...
    "url='https://github.com/kennyfrc/opencode'",
    "arch=('aarch64' 'x86_64')",
    "license=('MIT')",
    "provides=('opencode')",
    "conflicts=('opencode')",
    "depends=('fzf' 'ripgrep')",
    "",
    `source_aarch64=("\${pkgname}_\${pkgver}_aarch64.zip::https://github.com/kennyfrc/opencode/releases/download/v\${pkgver}\${_subver}/opencode-linux-arm64.zip")`,
    `sha256sums_aarch64=('${arm64Sha}')`,
    "",
    `source_x86_64=("\${pkgname}_\${pkgver}_x86_64.zip::https://github.com/kennyfrc/opencode/releases/download/v\${pkgver}\${_subver}/opencode-linux-x64.zip")`,
  ...
  const sourcePkgbuild = [
    ...
    "",
    `source=("opencode-\${pkgver}.tar.gz::https://github.com/kennyfrc/opencode/archive/v\${pkgver}\${_subver}.tar.gz")`,
    `sha256sums=('SKIP')`,
    "",
    "build() {",
    `  cd "opencode-\${pkgver}"`,
  ...
  const homebrewFormula = [
    ...
    `  homepage "https://github.com/kennyfrc/opencode"`,
    `  version "${Script.version.split("-")[0]}"`,
    "",
    "  on_macos do",
    "    if Hardware::CPU.intel?",
    `      url "https://github.com/kennyfrc/opencode/releases/download/v${Script.version}/opencode-darwin-x64.zip"`,
    `      sha256 "${macX64Sha}"`,
    "",
    "      def install",
    ...
    "    end",
    "    if Hardware::CPU.arm?",
    `      url "https://github.com/kennyfrc/opencode/releases/download/v${Script.version}/opencode-darwin-arm64.zip"`,
    `      sha256 "${macArm64Sha}"`,
    "",
    "      def install",
    '        bin.install "opencode"',
    ...
    `      url "https://github.com/kennyfrc/opencode/releases/download/v${Script.version}/opencode-linux-x64.zip"`,
    `      sha256 "${x64Sha}"`,
    "      def install",
    '        bin.install "opencode"',
    "      end",
    ...
    `      url "https://github.com/kennyfrc/opencode/releases/download/v${Script.version}/opencode-linux-arm64.zip"`,
    `      sha256 "${arm64Sha}"`,
    "      def install",
    '        bin.install "opencode"',
    "      end",
